...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, x_norm, mean_x, var_x, gamma, eps)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x, x_norm, mean_x, var_x, gamma, eps = cache
    N = x.shape[0]
    #out = gamma*x_norm+beta
    ...
    #gamma*x_norm
    dgamma = np.sum(dgamma_xnorm * x_norm, axis=0)
    dx_norm = dgamma_xnorm * gamma
    
    #x_norm = (x-mean_x)*(1/np.sqrt(var_x+eps))
    dx_mean_x1 = dx_norm * (1/np.sqrt(var_x+eps))
    d1_sqrt_var_eps = np.sum(dx_norm * (x-mean_x), axis=0)
    
    #1/np.sqrt(var_x+eps)
    dsqrt_var_eps = -1*(1/(var_x+eps))*d1_sqrt_var_eps

    #np.sqrt(var_x+eps)
    dvar_x = 0.5 * (1/np.sqrt(var_x+eps)) * dsqrt_var_eps
    
    #var_x = (1/N)sum(x-mean_x)**2
    dx_mean_x_sq = (1. / N) * np.ones(x.shape) * dvar_x
    
    #(x-mean_x)**2
    dx_mean_x2 = 2*(x-mean_x)*dx_mean_x_sq
    
    dx_mean_x = dx_mean_x1 + dx_mean_x2
    
    #x-mean_x
    dx1 = dx_mean_x
    dmean_x = -np.sum(dx_mean_x, axis=0)
    
    #mean_x = (1/n)(sum x)
    dx2 = (1. / N) * np.ones(x.shape) * dmean_x
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in xrange(N):
        x_pad = np.pad(x[i], [(0, 0), (pad, pad), (pad, pad)], 'constant')
        ...
        for j in xrange(H_prime):
          for k in xrange(W_prime):
            ...
            local_receptive_field = x_pad[:, h_start:h_end, w_start:w_end]
...
def conv_backward_naive(dout, cache):
    ...
    for i in xrange(N):
        x_pad = np.pad(x[i], [(0, 0), (pad, pad), (pad, pad)], 'constant')
        dx_pad = np.pad(dx[i], [(0, 0), (pad, pad), (pad, pad)], 'constant')
        ...
        for j in xrange(H_prime):
          for k in xrange(W_prime):
            ...
            for f in xrange(F):
              dx_pad[:, h_start:h_end, w_start:w_end] += w[f] * dout[i, f, j, k]
    ...
    for i in xrange(N):
        for j in xrange(H_prime):
          for k in xrange(W_prime):
            ...
            out[i, :, j, k] = np.amax(x[i, :, h_start:h_end, w_start:w_end].reshape((C, -1)), axis=1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
        ...
        for j in xrange(H_prime):
          for k in xrange(W_prime):
            ...
            for c in xrange(C):
              ...
              dx[i, c, h_start:h_end, w_start:w_end] = dout[i, c, j, k] * zeros
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
