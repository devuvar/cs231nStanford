...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in xrange(1,len(all_layers)):
            self.params['W'+str(i)] = weight_scale * np.random.randn(all_layers[i-1], all_layers[i])
            self.params['b'+str(i)] = np.zeros(all_layers[i])
    ...
    def loss(self, X, y=None):
        ...
        for i in xrange(1, self.num_layers + 1):
          ...
          scores, affine_cache = affine_forward(scores, W, b)
          ...
          if i < self.num_layers:
            if self.use_batchnorm:
              ...
              scores, batchnorm_cache = batchnorm_forward(scores, gamma, beta, bn_p)
